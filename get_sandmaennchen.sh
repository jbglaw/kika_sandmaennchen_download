...
if [ "${TIME}" -lt 1850 ]; then
	...
	THE_DAY_FOR_GREP="`date --date='1 day ago' '+%d%m%Y'`"
else
	THE_DAY="`date '+%Y%m%d'`"
fi
T3="`tempfile`"
...
if [ "${TIME}" -lt 1850 ]; then
	FILEPATH="`grep 'a.*href.*sendungen.*ipg.*_date-'"${THE_DAY_FOR_GREP}"'.*html' "${T3}" | cut -f 2 -d '"'`"
	wget -q -O "${T3}" "http://www.kika.de${FILEPATH}"
fi
